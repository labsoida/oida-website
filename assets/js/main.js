(function () {
  ...
  // --- Highlight della riga in lettura nelle liste lunghe (blog, casi studio) ---
  (function () {
    if (prefersReducedMotion) return;
    document.querySelectorAll('.blog-rows').forEach(function (list) {
      var rows = [].slice.call(list.querySelectorAll('.blog-row'));
      if (rows.length < 3) return;
      list.classList.add('sh-active');
      var ticking = false;
      function update() {
        ...
        for (var i = 0; i < rows.length; i++) {
          ...
          var m = r.top + r.height / 2, d = Math.abs(m - line);
          if (d < bd) { bd = d; best = rows[i]; }
        }
        for (var j = 0; j < rows.length; j++) rows[j].classList.toggle('is-reading', rows[j] === best);
      }
